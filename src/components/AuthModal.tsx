...
export function AuthModal() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // If popup works, modal closes automatically via context
    } catch (err: unknown) {
      const firebaseError = err as { code?: string };
      // Don't show error for popup-closed (user cancelled) or redirect (page will reload)
      if (
        firebaseError.code !== "auth/popup-closed-by-user" &&
        firebaseError.code !== "auth/cancelled-popup-request"
      ) {
        setError("Google sign-in failed. Make sure Google sign-in is enabled in your Firebase Console (Authentication → Sign-in method → Google).");
      }
    } finally {
      setGoogleLoading(false);
    }
      ...
      <div className="relative w-full max-w-md animate-[scaleIn_0.3s_ease-out]">
        ...
        <div className="relative bg-[#111118] border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
          ...
          <div className="relative px-6 pt-8 pb-4 text-center">
            ...
            <button
              onClick={closeModal}
              className="absolute top-4 right-4 w-8 h-8 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-white hover:bg-white/10 transition-all z-10"
            >
            ...
            <img
              ...
              alt="Bugle Boy Radio"
              className="w-16 h-16 mx-auto rounded-2xl shadow-lg shadow-amber-500/15 mb-4 object-contain relative"
            />
          ...
          <div className="px-6 pb-6">
            ...
            <button
              ...
              className={cn(
                ...
                "active:scale-[0.98]",
                (googleLoading || isSubmitting) && "opacity-60 cursor-not-allowed"
              )}
            >
              {googleLoading ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              {error && (
                <div className="flex items-start gap-2 px-3 py-2.5 bg-red-500/10 border border-red-500/20 rounded-lg">
                  <svg className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
