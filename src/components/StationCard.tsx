...
export function StationCard({
  station, isActive, isPlaying, isLoading, isFavorite, isDark, onPlay, onToggleFavorite,
}: StationCardProps) {
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className={cn("absolute inset-0 transition-opacity duration-500", isActive ? "opacity-100" : "opacity-0")}>
      ...
      <div className={cn(
        ...
        isDark
          ? "bg-white/[0.04] border border-white/[0.08] group-hover:bg-white/[0.07] group-hover:border-white/[0.12]"
          : "bg-white border border-gray-200 group-hover:bg-gray-50 group-hover:border-gray-300 shadow-sm group-hover:shadow-md"
      )} />
      ...
      >
        ...
        <div className={cn(
          ...
          isActive ? "bg-white/20 shadow-lg backdrop-blur-sm" : cn("bg-gradient-to-br shadow-md", station.color)
        )}>
          {isActive && isLoading ? (
            ...
            <div className="flex items-end gap-[3px] h-6">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="w-[3px] bg-white rounded-full animate-eq" style={{ animationDelay: `${i * 0.15}s` }} />
              ))}
            </div>
          ) : (
        ...
        <div className="flex-1 min-w-0 pr-6">
          <div className="flex items-center gap-2">
            <h3 className={cn(
              ...
              isActive ? "text-white" : isDark ? "text-white/90 group-hover:text-white" : "text-gray-800 group-hover:text-gray-900"
            )}>
              {station.name}
            </h3>
            {isActive && isPlaying && (
          ...
          <p className={cn("text-xs mt-1 truncate", isActive ? "text-white/70" : isDark ? "text-white/40" : "text-gray-500")}>
            {station.description}
          </p>
          <div className="flex items-center gap-2 mt-1.5">
            <span className={cn(
              "text-[10px] px-2 py-0.5 rounded-full font-semibold tracking-wide uppercase",
              isActive ? "bg-white/20 text-white" : isDark ? "bg-white/[0.07] text-white/50" : "bg-gray-100 text-gray-500"
            )}>
              {station.genre}
            </span>
            <span className={cn("text-[10px] truncate flex items-center gap-1", isActive ? "text-white/60" : isDark ? "text-white/30" : "text-gray-400")}>
        ...
        <div className="flex-shrink-0">
          {isActive && isPlaying ? (
            ...
            <div className={cn(
              ...
              isActive ? "bg-white/20" : isDark ? "bg-white/[0.05] group-hover:bg-white/10 group-hover:scale-110" : "bg-gray-100 group-hover:bg-gray-200 group-hover:scale-110"
            )}>
              <svg className={cn("w-4 h-4 ml-0.5 transition-colors", isActive ? "text-white" : isDark ? "text-white/50 group-hover:text-white/80" : "text-gray-400 group-hover:text-gray-600")} viewBox="0 0 24 24" fill="currentColor">
